...
class MonthForm extends CFormModel
{
    ...
    public function retrieveData($index,$city) {
        ...
        if(count($rows)==64){
            ...
            $e118=($c118>0.3?0:($c118>0.2?1:($c118>0.1?3:5)));
            //$e119=($c119>0.6?1:($c119>0.2?3:5));
            //IF(B48/B78>=0.1,IF(C119>0.2,1,IF(C119>0.1,3,IF(C119>=0,1,5))),IF(C119>0.2,3,IF(C119>0.1,5,IF(C119>=0,1,1))))
            $e119= ($b48/($b78==0?1:$b78)) >= 0.1 ? ( ($c119)>0.2?1:($c119>0.1?3:($c119>=0?5:0)) ) :( ($c119)>0.2?3:($c119>0.1?5:($c119>=0?1:0)));
            $e120=($c120>0.15?0:($c120>0.1?1:($c120>0.05?3:5)));
            $e121=($c121>1?5:($c121>0.8?3:1));
            $e122=($c122>1?5:($c122>0.8?3:1));
            $e124=($c124>0.3?0:($c124>0.2?1:($c124>0.1?3:5)));
...
//        print_r($msg_url);
    ...
    */
    ...
    public function retrieveDatas($model){
        ...
        elseif (count($model->record)==71){
            $path = Yii::app()->basePath.'/commands/template/m_template_iaib.xlsx';
        }
        else{
