...
class CheckInMonthForm extends CFormModel
{
    /* User Fields */
    public $start_date;
    public $end_date;
    ...
    public function retrieveData() {
        ...
        //签到签离统计
        $uCheckInMonth = CountSearch::getUCheckInMonth($startDate,$endDate,$city_allow);
        ...
        if($citySetList){
            ...
            foreach ($citySetList as $cityRow){
                ...
                if(isset($uCheckInMonth[$cityCode])){
                    foreach ($uCheckInMonth[$cityCode] as $officeID=>$uCityRow){
                        ...
                        if($officeID==$cityCode){
                            ...
                            $temp["city"] = $officeID;
                            $temp["city_name"] =  CheckInWeekForm::getOfficeNameByUID($officeID);
        ...
        $session['checkInMonth_c01'] = $this->getCriteria();
        $this->u_load_data['load_end'] = time();
        return true;
    }

    public function addTempByList(&$temp,$uLists){
        foreach ($uLists as $itemKey=>$itemVlaue){
            $uKey = $itemKey=="job_count"?"check_count":$itemKey;//特殊键名转换
    ...
    private function defMoreCity(){
        $list = array(
            ...
            "checkIn_abnormal_num"=>0,//签到异常次数(签到)
            "checkIn_abnormal_ratio"=>"",//签到异常比例(签到)
            ...
            "checkOut_abnormal_num"=>0,//签到异常次数(签离)
            "checkOut_abnormal_ratio"=>"",//签到异常比例(签离)
    ...
    protected function resetTdRow(&$list,$bool=false){
        if($bool){
            $ratioList = array(
                "checkIn_abnormal_ratio"=>"checkIn_abnormal_num",
                ...
                "checkOut_abnormal_ratio"=>"checkOut_abnormal_num",
            ...
            foreach ($ratioList as $key=>$item){
                ...
                if(!empty($list["check_count"])){
                    $list[$key] = round($list[$item]/$list["check_count"],4);
                    $list[$key] = ($list[$key]*100)."%";
                }
            }
        }
    }
    ...
    protected function getTopArr(){
        $topList=array(
            ...
            array("name"=>Yii::t("summary","Check In"),
                "colspan"=>array(
                    array("name"=>Yii::t("summary","Check Error Num")),//异常次数
                    array("name"=>Yii::t("summary","Check Error Radio")),//异常比例
    ...
    private function getDataAllKeyStr(){
        $bodyKey = array(
            ...
            "check_count"=>array("show"=>true,"addType"=>true),
            "checkIn_abnormal_num"=>array("show"=>true,"addType"=>true),
            "checkIn_abnormal_ratio"=>array("show"=>true,"addType"=>false),
            "checkIn_forgot_num"=>array("show"=>true,"addType"=>true),
            "checkIn_forgot_ratio"=>array("show"=>true,"addType"=>false),
            "checkIn_address_num"=>array("show"=>true,"addType"=>true),
            "checkIn_address_ratio"=>array("show"=>true,"addType"=>false),
            "checkIn_positioning_num"=>array("show"=>true,"addType"=>true),
            "checkIn_positioning_ratio"=>array("show"=>true,"addType"=>false),
            "checkOut_abnormal_num"=>array("show"=>true,"addType"=>true),
            "checkOut_abnormal_ratio"=>array("show"=>true,"addType"=>false),
            "checkOut_forgot_num"=>array("show"=>true,"addType"=>true),
            "checkOut_forgot_ratio"=>array("show"=>true,"addType"=>false),
            "checkOut_address_num"=>array("show"=>true,"addType"=>true),
            "checkOut_address_ratio"=>array("show"=>true,"addType"=>false),
            "checkOut_positioning_num"=>array("show"=>true,"addType"=>true),
            "checkOut_positioning_ratio"=>array("show"=>true,"addType"=>false),
            "spend_LT_5minute_count"=>array("show"=>true,"addType"=>true),
            "spend_LT_5minute_ratio"=>array("show"=>true,"addType"=>false),
            "spend_GT_240minute_count"=>array("show"=>true,"addType"=>true),
            "spend_GT_240minute_ratio"=>array("show"=>true,"addType"=>false),
        );
        return $bodyKey;
    }
    ...
    public function downExcel($excelData){
        ...
        $titleName = Yii::t("app","Check In Month");
