...
class SummaryBakForm extends CFormModel
{
	/* User Fields */
    ...
    protected function resetTdRow(&$list,$bool=false){
        ...
        if(SummaryBakForm::targetReadyBase()){
        ...
        if(SummaryBakForm::targetReadyUpside()){
        ...
        if(SummaryBakForm::targetReadyMinimum()){
    ...
    private function getTopArr(){
        ...
        $topList=array(
            ...
            array("name"=>Yii::t("summary","总生意额"),"rowspan"=>2),//新增服务(除一次性服务)
            array("name"=>Yii::t("summary","自己签自己做"),"rowspan"=>2),//一次性服务
           array("name"=>Yii::t("summary","自己签别人做"),"rowspan"=>2),
           array("name"=>Yii::t("summary","别人签自己做"),"rowspan"=>2),
        ...
        foreach ($officeList as $city=>$row){
            ...
            foreach ($row as $key=>$officeRow){//u_invoice_num
                ...
                if(key_exists($uKey,$uServiceMoneyV28)){
                    ...
                    $officeRow["u_v28_normal"]=$uServiceMoneyV28[$uKey]["normal"]+$officeRow["u_invoice_num"];//非交叉（自己签自己做）
