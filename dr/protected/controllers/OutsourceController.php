...
class OutsourceController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', 
				...
				'expression'=>array('OutsourceController','allowReadWrite'),
			),
			array('allow', 
				'actions'=>array('index','view','downExcel','ajaxDetail'),
				'expression'=>array('OutsourceController','allowReadOnly'),
			),
    ...
    public function actionAjaxDetail(){
        if(Yii::app()->request->isAjaxRequest) {//是否ajax请求
            $model = new OutsourceTable();
            $html =$model->ajaxDetailForHtml();
            echo CJSON::encode(array('status'=>1,'html'=>$html));//Yii 的方法将数组处理成json数据
        }else{
            $this->redirect(Yii::app()->createUrl('outsource/index'));
        }
    }

    public function actionDownExcel()
    {
        $model = new OutsourceForm('view');
        if (isset($_POST['OutsourceForm'])) {
            $model->attributes = $_POST['OutsourceForm'];
	...
	{
		$model = new OutsourceForm('index');
        $session = Yii::app()->session;
        if (isset($session['outsource_c01']) && !empty($session['outsource_c01'])) {
            $criteria = $session['outsource_c01'];
	...
	public static function allowReadWrite() {
		return Yii::app()->user->validRWFunction('G28');
	}
	
	public static function allowReadOnly() {
		return Yii::app()->user->validFunction('G28');
	}
}
