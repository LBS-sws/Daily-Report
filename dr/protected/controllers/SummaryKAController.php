<?php
//強制刷新文件
class SummaryKAController extends Controller
{
	public $function_id='G25';
	...
	public function accessRules()
	{
		return array(
			array('allow', 
				...
				'expression'=>array('SummaryKAController','allowReadWrite'),
			),
			array('allow', 
				'actions'=>array('index','view','downExcel','ajaxDetail'),
				'expression'=>array('SummaryKAController','allowReadOnly'),
			),
    ...
    public function actionAjaxDetail(){
        if(Yii::app()->request->isAjaxRequest) {//是否ajax请求
            ...
            $this->redirect(Yii::app()->createUrl('summaryKA/index'));
        }
    }

	public function actionIndex()
	{
		$model = new SummaryKAForm('index');
	...
	{
        set_time_limit(0);
        $model = new SummaryKAForm('view');
        if (isset($_POST['SummaryKAForm'])) {
            $model->attributes = $_POST['SummaryKAForm'];
            if ($model->validate()) {
                $model->retrieveData();
	...
	public static function allowReadWrite() {
		return Yii::app()->user->validRWFunction('G25');
	}
	
	public static function allowReadOnly() {
		return Yii::app()->user->validFunction('G25');
	}
}
