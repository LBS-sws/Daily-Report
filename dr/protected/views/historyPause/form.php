<?php
$this->pageTitle=Yii::app()->name . ' - HistoryPause Form';
?>

<?php $form=$this->beginWidget('TbActiveForm', array(
'id'=>'HistoryPause-form',
...
<section class="content-header">
	<h1>
        <strong><?php echo Yii::t('app','History Pause'); ?></strong>
...
<section class="content">
	...
	<div class="btn-group" role="group">
		<?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
				'submit'=>Yii::app()->createUrl('historyPause/index')));
		?>
	</div>
            <div class="btn-group pull-right" role="group">
                <?php echo TbHtml::button('<span class="fa fa-download"></span> '.Yii::t('dialog','Download'), array(
                    'submit'=>Yii::app()->createUrl('historyPause/downExcel')));
                ?>
            </div>
	</div></div>
                        ...
                        <div class="row panel panel-default" style="border-color: #333">
                            ...
                            <div class="panel-heading">
                                <h3 style="margin-top:10px;">
                                    <?php echo Yii::t('app','History Pause'); ?>
                            ...
                            <div class="table-responsive">
                                <?php echo $model->historyPauseHtml();?>
                            </div>
                        </div>
                    </div>
                </div>
...
$js="
    $('.click-th').click(function(){
        ...
        if($(this).hasClass('active')){
            ...
            $('#historyPause>thead>tr').eq(0).children().slice(startNum,endNum).each(function(){
                var width = $(this).data('width')+'px';
                $(this).width(width);
            });
            $('#historyPause>thead>tr').eq(2).children().slice(startNum-contNum,endNum-contNum).each(function(){
                $(this).children('span').text($(this).data('text'));
            });
            $('#historyPause>tbody>tr').each(function(){
