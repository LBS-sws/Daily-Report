<tr>
	<td>
		<?php echo TbHtml::dropDownList($this->getFieldName('task'),  $this->record['task'], General::getTaskList(),
			array('disabled'=>!Yii::app()->user->validRWFunction('A05')||$this->model->getReadonly(),'class'=>'select2','style'=>'width: 100%')
		); ?>
	</td>
	<td>
		<?php echo TbHtml::textField($this->getFieldName('qty'), $this->record['qty'], 
			array('readonly'=>!Yii::app()->user->validRWFunction('A05')||$this->model->getReadonly(),
	...
	<td>
		<?php echo TbHtml::dropDownList($this->getFieldName('finish'), $this->record['finish'], array('N'=>Yii::t('misc','No'),'Y'=>Yii::t('misc','Yes')),
							array('disabled'=>!Yii::app()->user->validRWFunction('A05')||$this->model->getReadonly())
		); ?>
	</td>
	<td>
		<?php 
			echo Yii::app()->user->validRWFunction('A05')&&!$this->model->getReadonly()
