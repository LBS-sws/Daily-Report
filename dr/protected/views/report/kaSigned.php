...
<section class="content">
    <div class="box"><div class="box-body">
            <div class="btn-group" role="group">
                <?php echo TbHtml::button(Yii::t('misc','Submit'), array(
                    'submit'=>Yii::app()->createUrl('report/kaSigned')));
                ?>
            </div>
        </div></div>
    ...
    <div class="box box-info">
        <div class="box-body">
            ...
            <?php endif ?>
            <div class="form-group">
                <?php echo $form->labelEx($model,'staff_list',array('class'=>"col-sm-2 control-label")); ?>
                <div class="col-sm-9">
                    <?php
                    $item = ReportKaSignedForm::getGroupStaffList();
                    if (empty($model->staff_list)) {
                        $model->staff_list = array();
                        foreach ($item as $key=>$value) {$model->staff_list[] = $key;}
                    }
                    echo $form->dropDownList($model, 'staff_list', $item,
...
$disabled = 'false';
$js = <<<EOF
$('#ReportKaSignedForm_staff_list').select2({
	tags: false,
	multiple: true,
