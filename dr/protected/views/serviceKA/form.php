<?php
$this->pageTitle=Yii::app()->name . ' - Service KA Form';
?>
...
<?php endif ?>
	<?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
		'submit'=>Yii::app()->createUrl('serviceKA/index'))
	); ?>
<?php if ($model->scenario!='view'): ?>
	<?php echo TbHtml::button('<span class="fa fa-upload"></span> '.Yii::t('misc','Save'), array(
		'submit'=>Yii::app()->createUrl('serviceKA/save'))
	); ?>
<?php endif ?>
...
<?php endif ?>
	<?php
		$counter = ($model->no_of_attm['serviceka'] > 0) ? ' <span id="docserviceka" class="label label-info">'.$model->no_of_attm['serviceka'].'</span>' : ' <span id="docserviceka"></span>';
		echo TbHtml::button('<span class="fa  fa-file-text-o"></span> '.Yii::t('misc','Attachment').$counter, array(
        'name'=>'btnFile','id'=>'btnFile','data-toggle'=>'modal','data-target'=>'#fileuploadserviceka',)
		);
            ...
            <?php if ($model->scenario!='new'): ?>
                <div class="btn-group pull-right" role="group">
                    ...
                    if (Yii::app()->user->validRWFunction('D07')){
                        echo TbHtml::button('<span class="fa fa-copy"></span> '.Yii::t('service','Copy To Service'), array(
                                'submit'=>Yii::app()->createUrl('serviceKA/copy',array("index"=>$model->id)))
                        );
                    }
                    ?>
                </div>
...
<?php $this->renderPartial('//serviceKA/historylist',array("model"=>$model)); ?>
...
$link3 = Yii::app()->createAbsoluteUrl("serviceKA/getcusttypelist");
$js = <<<EOF
$('#ServiceKAForm_cust_type').on('change',function() {
	var group = $(this).val();
	...
	$.ajax({
		...
		success: function(data) {
			$('#ServiceKAForm_cust_type_name').html(data);
		},
...
$fields = ($model->status=='N' || $model->status=='C') ? array('contact'=>'ServiceKAForm_cont_info',) : array();
...
$js = Script::genLookupButtonEx('btnServiceB4', 'product', 'b4_product_id', 'ServiceKAForm_b4_service');
...
$js = "
$('#ServiceKAForm_ctrt_period').on('change',function() {
	var end_dt = $('#ServiceKAForm_ctrt_end_dt').val();
	var sign_dt = $('#ServiceKAForm_sign_dt').val();
	var p = $(this).val();
	if (!end_dt && sign_dt && p) {
		var m = parseInt($('#ServiceKAForm_ctrt_period').val());
		var x = sign_dt.replace(/\//g,'-')
		var sd = new Date(x);
		$('#ServiceKAForm_ctrt_end_dt').val(addMonth(sd,parseInt(m)));
	}
});

$('#ServiceKAForm_sign_dt').on('change',function() {
	var end_dt = $('#ServiceKAForm_ctrt_end_dt').val();
	var period = $('#ServiceKAForm_ctrt_period').val();
	if (!end_dt && period) {
		var sd = new Date($('#ServiceKAForm_sign_dt').val().replace(/\//g,'-'));
		$('#ServiceKAForm_ctrt_end_dt').val(addMonth(sd,parseInt(period)));
	}
});
...
function redirection(arg) {
	...
	switch (arg) {
		case 'N':
			if (index==0)
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/new')."',{});
			else
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/new')."?index='+index,{});
			break;
		case 'A':
			if (index==0)
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/amend')."',{});
			else
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/amend')."?index='+index,{});
			break;
		case 'S':
			if (index==0)
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/suspend')."',{});
			else
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/suspend')."?index='+index,{});
			break;
		case 'R':
			if (index==0)
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/resume')."',{});
			else
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/resume')."?index='+index,{});
			break;
		case 'T':
			if (index==0)
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/terminate')."',{});
			else
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/terminate')."?index='+index,{});
			break;
		case 'C':
			if (index==0)
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/renew')."',{});
			else
				jQuery.yii.submitForm(elm,'".Yii::app()->createUrl('serviceKA/renew')."?index='+index,{});
			break;
	}
}
";
...
$js = Script::genDeleteData(Yii::app()->createUrl('serviceKA/delete'));
...
if ($model->status=='T') {
    $js = "
	 document.getElementById('ServiceKAForm_all_number').addEventListener('input',function(event){
           event.target.value = event.target.value.replace(/\-/g,''); 
        });
    document.getElementById('ServiceKAForm_surplus').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_all_number_edit0').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_all_number_edit1').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_all_number_edit2').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_all_number_edit3').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_surplus_edit0').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_surplus_edit1').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_surplus_edit2').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
    document.getElementById('ServiceKAForm_surplus_edit3').addEventListener('input',function(event){
       event.target.value = event.target.value.replace(/\-/g,''); 
    });
	";
